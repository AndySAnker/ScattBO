...
def BruteForce(atom='Au', simulated_or_experimental='simulated', scatteringfunction="Gr"):
    ...
    pH_values = np.arange(0, 14, 0.5)
    pressure_values = np.arange(0, 100, 0.5)
    solvent_values = ['Ethanol', 'Methanol', 'Water', 'Other']
    ...
    for pH in pH_values:
        for pressure in pressure_values:
            for solvent in solvent_values:
                ...
                if not any(d['structure_type'] == cluster.structure_type and d['number_of_atoms'] == len(cluster) and d['pressure'] == pressure for d in result):
...
def save_list_of_tuples(list_of_tuples, file_name):
    with open(file_name, 'w') as f:
        # Convert numpy types to native Python types
        converted_list = [{k: v.item() if isinstance(v, np.generic) else v for k, v in d.items()} for d in list_of_tuples]
...
save_list_of_tuples(result, '../../results/bruteforce/result_simulated_Gr_SynthesisSpace.json')
...
save_list_of_tuples(result, '../../results/bruteforce/result_simulated_Sq_SynthesisSpace.json')
...
save_list_of_tuples(result, '../../results/bruteforce/result_experimental_Gr_SynthesisSpace.json')
...
save_list_of_tuples(result, '../../results/bruteforce/result_experimental_Sq_SynthesisSpace.json')
